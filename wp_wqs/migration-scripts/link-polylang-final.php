<?php
/**
 * Properly link EN-ZH posts using Polylang structure with element_id
 */
...
echo "=== Linking EN-ZH posts via Polylang (with element_id) ===\n\n";

// Language term IDs from Polylang
$lang_en = 277;
$lang_zh = 280;

// Recreate table with correct structure including element_id
...
foreach ($jf_mapping as $ref_id => $titles) {
    ...
    // Create Polylang translation entry with element_id
    // translation_id = source post ID (English post ID)
    $pdo->prepare("INSERT INTO wp_pll_translations (translation_id, language, source_language, element_id) VALUES (?, 'en', NULL, ?)")->execute([$en_wp_id, $en_wp_id]);
    $pdo->prepare("INSERT INTO wp_pll_translations (translation_id, language, source_language, element_id) VALUES (?, 'zh', 'en', ?)")->execute([$en_wp_id, $zh_wp_id]);
...
$stmt_test = $pdo->query("SELECT * FROM wp_pll_translations WHERE element_id IN (186, 1600)");
while ($row = $stmt_test->fetch()) {
    echo "  id={$row['id']}, trans_id={$row['translation_id']}, lang={$row['language']}, elem_id={$row['element_id']}\n";
}