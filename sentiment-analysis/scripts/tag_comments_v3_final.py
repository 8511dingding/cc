...
def classify_comment(text: str, current_cognitive: str, current_emotion: str, current_behavior: str) -> Tuple[str, str, str]:
    ...
    if has_anger or is_long_essay:
        new_cog = '泛化抵触'
        new_emo = '愤怒背叛'
        new_beh = current_behavior if current_behavior != '暂无行动' else '暂无行动'
...
def run_tagging_v3(input_file: str, output_file: str = None):
    """运行v3标记流程"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        input_f = sys.argv[1]
        output_f = sys.argv[2]
        run_tagging_v3(input_f, output_f)